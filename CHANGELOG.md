...
### Added
- **会话回放内嵌 + 重放执行（用户口径，修复验证最后一环）**
  - 内嵌回放：质检详情抽屉内完整会话回放（每轮客户/客服对话 + 来源徽标 + 该轮链路步骤条），问题轮标红；操作列"回放"直达并自动展开，不再跳转审计页
  - 重放执行：`POST /quality/replay` 取原会话全部客户消息按原顺序重发到新会话（走真实消息管线：审计/闸门/队列全生效，per-session worker 串行保轮序）；详情抽屉一键触发 → 进度轮询 → 完成后自动结束重放会话触发质检 → **前后对比视图**（原/重放逐轮并排，回复变化行标绿 + 变化计数）——"同样的输入过当前链路"的修复验证口径
  - E2E：逾期问题重放，新旧回复对比 1/1 轮变化（均为 knowledge 来源、措辞演进），重放会话自动入质检

### Changed
- **整改闭环抽屉专家优化（用户口径）**
  - 顶部整改进度步骤条：归因 → 人工确认 → 修复(灰度) → 验证，处置状态机一眼可见；驳回态横幅提示
  - 头部徽标化：修复状态 + 信号源 + 出现次数（替代截断的用户输入标题）
  - 修复指引：分流表(A知识库/B意图库/C规则/D模型)带说明 tooltip，选定后显示"去哪里改什么"+ 页面直达链接（FAQ 管理/意图库/文档管理）
  - 操作分主次编号：①裁判归因 → ②确认进入修复 → ③转灰度 → ④全量上线 → ⑤复检原会话（灰度即可复检验证，复检结果直接提示"修复验证通过/仍有问题建议回退"）；流转弹窗强制经过备注输入（fix_note 可追溯）
  - E2E：未归因空态/已归因指引/驳回横幅三形态浏览器实测

### Changed
- **质检工作台大改版：双页签合并为统一会话视图 + 问题导向详情（用户口径）**
  - 统一会话列表：质检判定 ⟕ 问题案例全外联（每侧均按会话取最新），一行一会话；分类筛选 正常/提醒/判定有问题/待人工判定/未质检（分类 CASE：待人工案例优先于判定，未检案例可检索）；分类统计卡可点选（待复核/判定有问题/正常）；批量确认/批量流转保留（仅携带案例的行可选）
  - 列：会话时间（三锚点 tooltip）/ 会话 ID / 轮数 / 判定+待人工 / 质检问题 / 信号 / 根因层 / 修复状态 / 操作（详情·回放·整改）
  - 问题导向详情（专家方案落地）：裁判摘要 → 问题定位卡（标签+轮次+原因+**该轮对话现场还原**（客户原话/客服回复/来源徽标）+**该轮链路**（出队→分类→路由→… 压缩步骤条））→ 会话全景（缩略时间线，问题轮标红）→ 质检信息 → 处置（会话回放/整改闭环/**复检此会话**）；轮号口径对齐裁判（按对话日志行计数，向前/向后定位客户句与客服句）
  - 新端点：`GET /quality/sessions`（统一列表）+ `POST /quality/rescan`（单会话强制复检，绕过 30 天去重，实测 8s 出判定；整改效果验证闭环）
  - E2E：fail 会话 406 筛选命中；问题轮现场（"对,挂失"→"没太理解"[clarify]）与复检按钮实测通过

### Changed
- **质检列表会话维度 + 问题导向详情（用户口径）**
  - 列表按会话聚合：每会话只展示最新一次判定（row_number 窗口去重，方言中立），复检多行不再重复出现；筛选时取该会话最新一条匹配记录（曾不合格复检转合格的会话仍可检索到）；总数=会话数（API 实测与 DB DISTINCT 一致 3403）
  - 查询页移除"首轮客户输入"列，换会话 ID 标识列（内容搜索能力保留）
  - 新增质检详情抽屉（问题导向：判定徽标 → 裁判摘要 → 问题定位（标签+轮次+原因）→ 质检信息 → 会话回放/整改闭环下钻），行点击直达
...
- **决策链整改（会话 smoke-qa-1788567861 复盘，5 项）**
  ...
  - 复合误判修复：`installment_inquiry`（咨询域）作对冲性弱次选即触发复合；检测加 0.30 分数线（与闲聊短路同纪律），弱次选不再触发，纯查询单次执行
  ...
  - 决策链顺序修复：PG 行 created_at 此前在后台 task 执行时才生成，同批留痕微秒级乱序（"路由决策"排到"意图分类"前）；改为 log_decision 调用时刻，回放顺序恢复语义（出队→分类→路由→工具→完成→诉求跟踪）

### Changed
- **决策链展示重构（对话审计回放）**：决策链按"第 N 轮"分组（轮徽标 + 全程耗时 + 步数），新增消息出队/诉求跟踪动作中文化，排队/工具调用/摘要生成分段耗时直读，dict 证据对象渲染修复
- 决策链分组存量兼容：修复前的会话每条决策独立 turn_id，按轮分组会拆成 N 个假轮次 — 无 turn_start 特征的决策合并为「历史记录 · 未分轮」组如实展示
- **执行链路命名收敛**：源码与测试中"链A/链B/链C/链D"字眼全部替换为描述性命名（交易链路/查询直达/复合级联/并行竞速），决策留痕文案同步（"链B直连" → "查询直达"）
