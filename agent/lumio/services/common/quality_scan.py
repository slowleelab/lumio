...
A. 答非所问 — 客服回复没有回答客户当前问的那个具体问题。判定标准是"对题"而非
   "关键词重叠": 客户问"为什么收年费", 客服答"积分可兑换航空里程/年费抵扣" 虽含
   "年费"字样, 但答成了另一个问题 → 判 A。沾边信息不等于回答。
...
F. 回复残缺 — 客服回复语句戛然而止/明显截断 (如以"请问您持有"结尾) 或只有半句,
   客户无法据此行动 → 至少 warn; 因残缺导致问题完全未答 → fail
...
- 追问必要信息 (账期/卡种) 后等待客户补充 — 前提: 客户表达确实含糊或缺该必要信息

以下必须判 fail, 不得豁免 (无效澄清, 会话 8d988206 复盘):
- 客户话语含明确业务诉求且语义完整 (如 "我想转账但限额怎么办" 这类关键词+诉求
  清晰的句子), 机器人却回复 "没听明白/请换个说法" 类澄清话术 = 答非所问 (A) —
  不得以 "客户表述含糊" 为由豁免; "机器人没听懂" 不能倒推 "客户没说清"。
  正确行为: 正面回答该业务问题, 或说明不支持该业务并给出替代指引, 或转人工。

输出 JSON (不要输出其他内容):
{"verdict": "pass|warn|fail",
 "problems": [{"type": "A|B|C|D|E|F", "turn": <轮次序号>, "reason": "<一句话证据, 引用客户原话与客服原话片段>"}],
 "summary": "<一句话总评>"}

判定纪律: warn = 轻微问题 (表达生硬/引导不足/回复略偏但有部分相关信息), fail =
A-F 任一成立且客户问题实质落空; **存疑不再判 pass — 证据不足以定级时至少 warn**
(质检宽松比误伤代价更高); 只有逐项审查全部无迹象才 pass。

判例 (校准锚点, 判定时对照):
- 客户: "我的卡为什么收年费" / 客服: "积分可兑换以下项目: 航空里程…年费抵扣: 按卡种"
  → fail (A): 含"年费"关键词但答成了积分兑换, 客户问的是收费原因
- 客户: "就是费年？" (表述含糊) / 客服: "您是想了解年费吗? 通常每年消费一定次数可免…"
  → pass: 澄清后给出对题信息
- 客户: "帮我查账单" / 客服: "您的账单金额为 3,200 元, 最低还款 320 元" → pass (工具直查正常服务)"""
