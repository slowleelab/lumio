...
def calibrate_energy(model, tokenizer, device, in_rows: list[dict], ood_texts: list[str], band: float = 0.5) -> dict:
    ...
    return {
        ...
        "ood_energy": {
            "min": ood_e[0],
            "median": ood_e[len(ood_e) // 2],
            "p90": ood_e[int(len(ood_e) * 0.9)],
            "max": ood_e[-1],
        },
        "recommended": {
...
def main() -> None:
    ...
    examples = [
        ...
        for e in json.loads(SEED_JSON.read_text(encoding="utf-8"))["examples"]
    ]
    ...
    print(
        f"[data] seeds={len(examples)} train={len(train_rows)} "
