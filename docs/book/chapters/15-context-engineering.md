...
**为什么是这个拼接顺序**: 五段不是按代码里出现的顺序随手排的, 而是按**一旦令牌被截断, 哪一段丢失对决策伤害最大**来倒推的——重要度越高越靠前, 因为靠前的内容在模型注意力的"近因"区域 (模型对提示词头部的关注度高于尾部), 越靠前越不容易被淹没. 逐段解释:

- **第一段是对话摘要**: 摘要丢失等于整个对话脉络归零——客户前二十轮谈了什么、承诺了什么、抱怨了什么, 模型完全失忆. 哪怕只剩一段, 也必须是它.
- **第二段是客户画像**: 卡种和会员等级决定**回答的语气和能给的方案**——白金卡客户能听"提升额度到八万", 普通卡客户听到就是空头支票. 画像丢了对贵宾客户的体验立刻崩.
- **第三段是实体池**: 已经抽出来的卡号、金额、日期是**事实数据**——一旦丢掉, 模型下一轮还得让客户重说一遍, 体验极差. 但实体本身可以在对话历史里重现 (15.3 的后进先出累加兜底), 所以排在画像后面.
- **第四段是意图历史**: 客户从"问年费"切到"办挂失"的轨迹, 给模型看的目的是**理解话题切换的合理性**. 这一段丢了, 模型顶多以为客户在"前后聊两件不相关的事", 影响有限.
- **第五段是当前意图**: 当前轮的意图分类结果, 在模型收到这条提示词的同时, **业务侧其实已经把当前意图传到了下游路由** (第 3 章的决策器六步已经分派完意图), 这段更像是给模型的"提示", 不是关键决策依据. 排在最后, 截断就截断, 不致命.

**为什么默认值不写入**:
- `vip_level != "普通"`: 普通就是默认值, 把"客户是普通会员"写进提示词, 模型反而要花算力去理解这个中性词, 属于噪声.
- `risk_tolerance != "R2"`: R2 在产品定义里就是中位档, 写出来等同于没写. 只有 R1 (保守) 或 R3 (激进) 这类偏离默认的才需要让模型注意.
- 实体池是**全量写入**: 实体一旦抽出来, 就是事实, 过滤反而会丢真值.

**互补设计**: 第一层注入的是**结构化记忆** (用中括号标签加等号串的键值对风格), 不含自然语言冗余, 五百个令牌内能表达一百多个实体. 跟第二层 (自然语言对话历史) 形成"结构化加非结构化"互补——结构化层告诉模型客户"是什么状态", 非结构化层告诉模型客户"具体怎么说的".
